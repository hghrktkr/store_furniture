...
export class ShoppingBasketManager {
  ...
  static onBreakShoppingBasketStand(ev: BlockComponentBlockBreakEvent) {
    const { brokenBlockPermutation, block } = ev;
    const amount = brokenBlockPermutation.getState("edu:basket_count" as keyof BlockStateSuperset);
  ...
  static onUseShoppingBasket(ev: ItemComponentUseOnEvent) {
    const { source } = ev;
    ...
    if (lookingBlock.typeId === "edu:shopping_basket_stand") {
      this.changeState(source as Player, lookingBlock);
      return;
    }
    ...
    this.consumeBasket(source as Player);
  }

  private static consumeBasket(player: Player) {
  ...
  private static changeState(player: Player, basket: Block) {
    const perm = basket.permutation;
    const currentState = perm.getState("edu:basket_count" as keyof BlockStateSuperset);
    if (typeof currentState !== "number") return;

    if (currentState < 5) {
      basket.setPermutation(perm.withState("edu:basket_count" as keyof BlockStateSuperset, currentState + 1));
